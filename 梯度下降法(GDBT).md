...
<h1>1. 梯度</h1>
在微积分里面，对多元函数的参数求$∂$偏导数，把求得的各个参数的偏导数以向量的形式写出来，就是梯度。比如函数$f(x,y)$, 分别对$x$,$y$求偏导数，求得的梯度向量就是$(∂f/∂x,∂f/∂y)^T$,简称grad f(x,y)或者▽f(x,y)。对于在点(x<sub>0</sub>,y<sub>0</sub>)的具体梯度向量就是(∂f/∂x<sub>0</sub>,&nbsp;∂f/∂y<sub>0</sub>)<sup>T</sup>.或者▽f(x<sub>0</sub>,y<sub>0</sub>)，如果是3个参数的向量梯度，就是(∂f/∂x,&nbsp;∂f/∂y，∂f/∂z)<sup>T</sup>,以此类推。
