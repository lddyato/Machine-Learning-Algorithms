...
# 1. 梯度
在微积分里面，对多元函数的参数求$∂$偏导数，把求得的各个参数的偏导数以向量的形式写出来，就是梯度。比如函数$f(x,y)$, 分别对$x$,$y$求偏导数，求得的梯度向量就是$(∂f/∂x,∂f/∂y)^T$,简称grad f(x,y)或者▽f(x,y)。对于在点$(x_0,y_0)$的具体梯度向量就是$(∂f/∂x_0$; $∂f/∂y_0)^T$.或者$▽f(x_0,y_0)$，如果是3个参数的向量梯度，就是$(∂f/∂x,∂f/∂y,∂f/∂z)^T$,以此类推。

那么这个梯度向量求出来有什么意义呢？他的意义从几何意义上讲，就是函数变化增加最快的地方。具体来说，对于函数![equation](http://latex.codecogs.com/gif.latex?f(x,y)),在点![equation](http://latex.codecogs.com/gif.latex?\(x_0, y_0))，沿着梯度向量的方向就是![equation](http://latex.codecogs.com/gif.latex?(∂f/∂x_0, ∂f/∂y_0)^T)的方向是![equation](http://latex.codecogs.com/gif.latex?f(x,y))增加最快的地方。或者说，沿着梯度向量的方向，更加容易找到函数的最大值。反过来说，沿着梯度向量相反的方向，也就是 -(∂f/∂x<sub>0</sub>,&nbsp;∂f/∂y<sub>0</sub>)<sup>T</sup>的方向，梯度减少最快，也就是更加容易找到函数的最小值。</p>
